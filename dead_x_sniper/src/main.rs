use shared::{pf::buy_ca_tg, tg::{client::connect_client, dialog::find_dialog::find_dialog_chat_by_id}, twitter_regex::extract_twitter_sender};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let redacted_bot_dialog_id:i64 = 
    env::var("REDACTED_SYSTEMS_BOT_DIALOG_ID")?.parse()?;

    let pepeboost_dialog_id:i64 =
    env::var("PEPEBOOST_DIALOG_ID")?.parse()?;

    let trojan_dialog_id:i64 =
    env::var("TROJAN_DIALOG_ID")?.parse()?;
    ...
    for (key,_) in &tracked_twitter_account {
        client.send_message(&redacted_bot_chat, format!("/add {}",key)).await?;
    }
